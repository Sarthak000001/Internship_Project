...
const Filter = () => {
    ...
    // Extract unique prices using Set
    const uniquePrices = [...new Set(product.map(item => item.price))];
    const uniqueCategorySet = new Set(product.map((item)=>item.category));
                    ...
                    <div>
                        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 mt-4">
                            <select value={filterType} onChange={(e)=>setFilterType((e.target.value.toLowerCase()))} className="px-4 py-3 w-full rounded-md bg-gray-50 border-transparent outline-0 focus:border-gray-500 focus:bg-white focus:ring-0 text-sm" style={{ backgroundColor: mode === 'dark' ? 'rgb(64 66 70)' : '', color: mode === 'dark' ? 'white' : '', }}>
                                {
                                   uniqueCategory &&  uniqueCategory.map((category,index)=>{
                            ...
                            <select value={filterPrice} onChange={(e)=>setFilterPrice(e.target.value.toLowerCase())} className="px-4 py-3 w-full rounded-md bg-gray-50 border-transparent outline-0  focus:border-gray-500 focus:bg-white focus:ring-0 text-sm" style={{ backgroundColor: mode === 'dark' ? 'rgb(64 66 70)' : '', color: mode === 'dark' ? 'white' : '', }}>
                                ...
                                {
                                    uniquePrices && uniquePrices.map((price,index)=>{
